 ...
 */
function eazyrecruitz_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'eazyrecruitz_unset_default_widgets_args' );
 ...
 */
function eazyrecruitz_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'eazyrecruitz_child_functions_php', 10, 2 );
 ...
 */
function eazyrecruitz_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'eazyrecruitz'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'eazyrecruitz_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'eazyrecruitz' ),
			'preview_url'              => 'http://commonsupport.net/newwp/eazyrecruitz/',
		),
	);
}

add_filter( 'merlin_import_files', 'eazyrecruitz_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function eazyrecruitz_after_import_setup() {
	// Assign menus to their locations.
	$main_menu = get_term_by( 'name', 'Main Menu', 'nav_menu' );
	$rtl_menu = get_term_by( 'name', 'RTL Menu', 'nav_menu' );
	$onepage_menu = get_term_by( 'name', 'One Page Menu', 'nav_menu' );

	set_theme_mod(
		'nav_menu_locations', array(
			'main_menu' => $main_menu->term_id,
			'rtl_menu' => $rtl_menu->term_id,
	...
	$blog_page_id  = get_page_by_title( 'Blog List View' );
...
add_action( 'merlin_after_all_import', 'eazyrecruitz_after_import_setup' );

/**
 * Eazyrecruitz 2 mime types.
 * 
 * @param  [type] $mimes [description]
 * @return [type]        [description]
 */
function eazyrecruitz_mime_types($mimes) {
 $mimes['svg'] = 'image/svg+xml';
 return $mimes;
}
add_filter('upload_mimes', 'eazyrecruitz_mime_types');